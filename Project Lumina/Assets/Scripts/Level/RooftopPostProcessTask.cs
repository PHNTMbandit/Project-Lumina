...
namespace ProjectLumina.Level
{
    ...
    public class RooftopPostProcessTask : DungeonGeneratorPostProcessingGrid2D
    {
        ...
        private void AddWallsUnderRooms(DungeonGeneratorLevelGrid2D level)
        {
            ...
            _wallsTilemap = tilemaps.Single(x => x.name == "Walls");
            _backgroundTilemap = tilemaps.Single(x => x.name == "Background");

            // Add walls under outside rooms
            foreach (var roomInstance in level.RoomInstances)
            {
                var room = (LuminaRoom)roomInstance.Room;
        ...
        private void AddWallsUnderRoom(RoomInstanceGrid2D roomInstance)
        {
            ...
            {
                var tilemap = _backgroundTilemap;
